...
def trajectory_3d(
    ...
    for result in results:
        ...
        figure.add_trace(go.Scatter3d(
            ...
            mode="lines", line={"width": 6, "color": color},
            name=f"actual · {result.mode}",
        ))
        figure.add_trace(go.Scatter3d(
            x=result.estimated_states[:, 0], y=result.estimated_states[:, 1],
            z=result.estimated_states[:, 2], mode="lines",
            line={"width": 2, "dash": "dot", "color": color},
            opacity=0.55, name=f"estimate · {result.mode}",
        ))
        if prediction_index is not None and len(result.predicted_trajectories):
            ...
            figure.add_trace(go.Scatter3d(
                x=prediction[:, 0], y=prediction[:, 1], z=prediction[:, 2],
                mode="lines+markers", marker={"size": 3},
                line={"width": 4, "dash": "dash", "color": color},
                name=f"horizon · {result.mode}",
            ))
    figure.add_trace(go.Scatter3d(
        x=[scenario.start[0]], y=[scenario.start[1]], z=[scenario.start[2]],
        mode="markers", marker={"size": 7, "color": "#8395a7"}, name="start",
    ))
    figure.add_trace(go.Scatter3d(
        x=[scenario.goal[0]], y=[scenario.goal[1]], z=[scenario.goal[2]],
        mode="markers", marker={"size": 10, "color": "#feca57", "symbol": "diamond"},
        name="goal",
    ))
...
def telemetry_figure(result: SimulationResult, scenario: ScenarioConfig):
    ...
    figure = make_subplots(
        rows=3, cols=2,
        subplot_titles=("Position", "Velocity", "Attitude", "Control", "Tracking error", "Cost decomposition"),
    )
    for index, label in enumerate(("x", "y", "z")):
        figure.add_trace(go.Scatter(x=result.times, y=result.states[:, index], name=label), row=1, col=1)
        figure.add_trace(go.Scatter(x=result.times, y=result.reference_positions[:, index], name=f"{label} ref", line={"dash": "dot"}), row=1, col=1)
    for index, label in enumerate(("vx", "vy", "vz")):
        figure.add_trace(go.Scatter(x=result.times, y=result.states[:, 3 + index], name=label), row=1, col=2)
    for index, label in enumerate(("roll", "pitch", "yaw")):
        figure.add_trace(go.Scatter(x=result.times, y=np.degrees(result.states[:, 6 + index]), name=label), row=2, col=1)
    for index, label in enumerate(("roll cmd", "pitch cmd", "vz cmd", "yaw rate")):
        figure.add_trace(go.Scatter(x=result.times, y=result.controls[:, index], name=label), row=2, col=2)
    ...
    for name, values in result.cost_terms.items():
        if name != "total":
            figure.add_trace(go.Scatter(x=result.times, y=values, name=name, connectgaps=True), row=3, col=2)
...
def safety_figure(result: SimulationResult):
    ...
    figure = make_subplots(rows=2, cols=2, subplot_titles=(
        "Clearance", "Chance residual", "Chance slack", "Position uncertainty",
    ))
    figure.add_trace(go.Scatter(x=result.times, y=result.clearances, name="clearance"), row=1, col=1)
    figure.add_trace(go.Scatter(x=result.times, y=result.chance_residuals, name="residual"), row=1, col=2)
...
def solver_figure(result: SimulationResult):
    ...
    figure.add_trace(go.Bar(
        x=result.times[iteration_mask], y=result.solver_iterations[iteration_mask], name="iterations",
    ), row=1, col=2)
...
def comparison_metrics(results: list[SimulationResult]):
    ...
    figure.add_trace(go.Bar(x=labels, y=[r.metrics.tracking_rmse_m for r in results], marker_color=colors, name="RMSE"), row=1, col=1)
    figure.add_trace(go.Bar(x=labels, y=[r.metrics.min_clearance_m or 0.0 for r in results], marker_color=colors, name="clearance"), row=1, col=2)
    figure.add_trace(go.Bar(x=labels, y=[r.metrics.p95_solver_ms for r in results], marker_color=colors, name="p95"), row=1, col=3)
...
def monte_carlo_distributions(results: list[SimulationResult]):
    ...
    for mode in sorted({item.mode for item in results}):
        ...
        figure.add_trace(go.Box(y=[r.metrics.tracking_rmse_m for r in items], name=mode, marker_color=color), row=1, col=1)
        clearance = np.sort(np.asarray([
            r.metrics.min_clearance_m for r in items
            if r.metrics.min_clearance_m is not None
        ], dtype=float))
        if clearance.size:
            figure.add_trace(go.Scatter(
                x=clearance,
