...
def continuous_dynamics(
    ...
    k_phi: float = 1.0,
    k_theta: float = 1.0,
    # FIX #20 (LOW): default was 1.0 but mpc.yaml sets k_vz=3.0 — unified to 3.0
    # so that bare function calls (not via QuadrotorDynamics.from_config) also
    # use the correct gain that matches the YAML configuration.
    k_vz: float = 3.0,
    tau_phi: float = 0.2,
    tau_theta: float = 0.2,
    tau_vz: float = 0.4,
