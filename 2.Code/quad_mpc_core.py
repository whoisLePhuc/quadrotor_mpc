...
def obstacle_pos_at(obs, t):
    ...
    y = obs['amp'] * np.sin(2*np.pi*t/obs['period'])
    return obs['x'], y, obs['z']
...
def build_controller(model, obstacles, dyn_idx, bounds, margin, n_horizon, dt,
                      max_iter=60, use_jit=False,
                      w_pos=(2, 2, 14), w_vel=(1, 1, 1.5), w_att=80.0,
                      w_rate=(2, 2, 0.5), r_ctrl=(0.05, 0.08, 0.08, 0.05),
                      penalty=1e4):
    mpc = do_mpc.controller.MPC(model)
    nlpsol_opts = {'ipopt.max_iter': max_iter, 'ipopt.tol': 1e-4,
                   'ipopt.acceptable_tol': 1e-3, 'ipopt.print_level': 0,
    ...
    if use_jit:
        ...
        nlpsol_opts.update({'jit': True, 'compiler': 'shell',
                             'jit_options': {'flags': ['-O1'], 'verbose': False}})
    mpc.set_param(
        n_horizon=n_horizon, t_step=dt, n_robust=0, store_full_solution=True,
    ...
    for i, obs in enumerate(obstacles):
        ...
        mpc.set_nl_cons(f'obstacle_{i}', safe_dist - dist_expr, ub=0,
                         soft_constraint=True, penalty_term_cons=penalty)
...
def make_mpc_tvp_fun(template, goal_state, obstacles, dyn_idx, n_horizon, dt):
    ...
    below)."""
    def tvp_fun(t_now):
        qg = quat_from_euler(goal_state['euler']['roll'], goal_state['euler']['pitch'], goal_state['euler']['yaw'])
...
def build_cached_mpc(bounds, obstacles, margin, n_horizon=20, dt=0.05, max_iter=60, use_jit=False):
    ...
    mpc = build_controller(model, obstacles, dyn_idx, bounds, margin, n_horizon, dt,
                            max_iter=max_iter, use_jit=use_jit)
    goal_state = {'pos': {'x': 0, 'y': 0, 'z': 1}, 'euler': {'roll': 0, 'pitch': 0, 'yaw': 0}}
    mpc.set_tvp_fun(make_mpc_tvp_fun(mpc.get_tvp_template(), goal_state, obstacles, dyn_idx, n_horizon, dt))
    mpc.setup()
    return model, mpc, dyn_idx, goal_state


def run_simulation(x0_vals, goal_pos, goal_euler, bounds, obstacles, margin,
                    sim_seconds=12.0, dt=0.05, n_horizon=20, max_iter=60,
                    progress_cb=None, cached=None):
    """
    ...
    if cached is not None:
        ...
        model, dyn_idx = build_model(dt, obstacles)
        mpc = build_controller(model, obstacles, dyn_idx, bounds, margin, n_horizon, dt, max_iter=max_iter)
    ...
    simulator.set_tvp_fun(make_sim_tvp_fun(simulator.get_tvp_template(), goal_state, obstacles, dyn_idx))
    simulator.setup()
